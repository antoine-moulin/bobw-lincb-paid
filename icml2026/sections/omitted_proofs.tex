...
Consider first estimate (i). Following Eq.\;(8) from \cite{neu20adversarial} we can use that, for all $i \in [K]$,  
\begin{align*}
...
\end{align}
Now to be able to do the same as for Lemma 6 of \cite{neu20adversarial} we would need to have $Z_t(x)/p_t(x)=1$, so the expectation could go in the trace and yield only a $d$ factor. \db{I don't know how to deal with this term, this looks rather necessary if we don't want to use a uniform probability.}  
