...
%
\db{I guess we should write everything with expectations here?}
\begin{align*}
...
\end{equation}
For the moment, forget about MGR and assume that the estimate is of the form $\wt \theta_{t,a} = \Sigma_{t,a}^{-1}X_t\ell_t(X_t, a)\ind(a\in O_t)$, where $\Sigma_{t,a}$ is any matrix that makes the estimator unbiased/with controlled bias. Then, following e.g the proof of Lemma 2 of \cite{BOBWlinear} we can write that 
\begin{align*}
	\abs{\inp{x, \wt \theta_{t,a}}} &\leq \norm{\Sigma_{t,a}^{-1}}_{\text{op}} \\
	& \leq  \frac{1}{\lambda_{\min}(\Sigma_{t,a})} \;, \\
\end{align*}
using that all contexts are in the unit Euclidean ball. Some examples are
\begin{itemize}
	\item[(i)]  $\Sigma_{t,a}=p_t(X_t) \Sigma^{-1}$, which yields $\lambda_{\min}(\Sigma_{t,a}) = p_t(X_t)\cdot \lambda_{\min}(\Sigma)$.
	\item[(ii)] $\Sigma_{t,a}=\E_{t, X\sim \cD}\left[p_t(X) XX^T\right]$, which gives $\Sigma_{t,a}\succeq \lambda_{\min}(\Sigma)\; \underset{x\in \cX}{ \min } \;p_t(x)\cdot  I$ so $\lambda_{\min}(\Sigma_{t,a}) \geq \underset{x\in \cX}{ \min }\; p_t(x)  \cdot \lambda_{\min}(\Sigma)$.
	\item[(iii)] MGR estimate of (ii), for which it is known (\cite{BOBWlinear}[Lemma 2]) that $\norm{\Sigma_{t,a}^+}_{\text{op}} \leq \frac{(M_t+1)}{2}$.
\end{itemize}
...
\[\forall x \in \cX, \; u_t(x) = \frac{4\eta_t}{1-\alpha} \cdot \max_{x\in \cX} q_{t\star}(x)^{1-\alpha} \;. \]
...
\db{An alternative path, but maybe super hard would be to upper bound the ratio $\sup_{x,y} \frac{p_t(x)}{p_t(y)}$.}

\db{Alternative alternative path: we can use the MGR to estimate the map of observation probabilities over contexts. Maybe we can use some sort of smoothness arguments to show that this would be enough to get a good tuning for $u_t$?}
...
\[\stab\leq \frac{4}{1-\alpha} \sum_{t=1}^T   \bE_t\left[ \sum_{i \neq \wt I_t} q_i(x)^{2-\alpha} \wt \ell_{t,i}(x)^2 + q_{t\star}(x)^{2-\alpha} \wt \ell_{t,\wt I_t}(x)^2 \right] \;. \]

To fit in the framework of \cite{BOBWhardproblems} we need to upper bound this term by $\frac{z_t}{\beta_t \gamma'_t}=\sqrt{\frac{z_t}{\beta_t}}$.


\paragraph{Controlling the penalty term.} \antoine{If I understand correctly the second regularizer becomes useful to enable the use of Lemma~\ref{lem::ftrl_smooth_bound}}
