...
& \leq \frac{d}{p_t(X_t)\lambda_{\min}} \;.
...
\end{align}
%\DB{Wait, can we do something like above which would make $\norm{\Sigma_t^{-1}}_{\text{op}}$ appear? And so wouldn't }
...
\]
What does it mean for us? Well, in both cases the LHS is properly defined, and I assume that for us the condition on the RHS should be taken in expectation over context, to satisfy a somewhat a self-bounding property when averaging over contexts. 
...
which is however somewhat super ugly. \DB{Question: is it however possible to replace this guy by something proportional to $\lambda_{\min}$ somehow? Might depend on the context distribution, but it'd feel kinda OK imo. Again, this relies on invoking some sort of smoothness of the probabilities in similar directions.}
