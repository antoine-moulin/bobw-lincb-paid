...
\end{align*}
using that all contexts are in the unit Euclidean ball. Some examples are $\Sigma_{t,a}^{-1}=\frac{\Sigma^{-1}}{p_t(X_t)}$, in which case the above bounds yields $\frac{1}{p_t(X_t)\lambda_{\min}(\Sigma)}$; or $\Sigma_{t,a}=\E_{t, X\sim \cD}\left[p_t(X) XX^T\right]$, which satisfies $\Sigma_{t,a}\succ \lambda(\Sigma) \min_{x\in \cX} p_t(x) I$.  For the MGR estimate of the later, it is known (\cite{BOBWlinear}[Lemma 2]) that the bound becomes $\frac{\eta_t (M_t+1)}{2}$, which encourages to tune $M_t$ at most of order $\frac{1}{\eta_t}$. This can in turn impose constraints on the tuning of $\eta_t$ (in that case the tuning of $u_t$ will appear in the control of the bias).  
...
\db{This is a bit hard to sell from a computational perspective. Can we find less ugly? If we replace by $1/2$ what do we lose? Do we still get BOBW but get uglier logs?}
